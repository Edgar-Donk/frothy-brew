...
from tkinter.ttk import Scrollbar, Scale, LabelFrame, Frame, Spinbox, Style, \
    Combobox, Label, Button, Progressbar
import sys
sys.path.append('../scale')
from gen_scale_class_dpi import TtkScale
import ctypes
...
def change_style(event=None):
    ...
    st0.configure('dpi.TSpinbox', arrowsize='6p') # element_options
    st0.configure('dpi.Horizontal.TScrollbar', arrowsize='15p')
    st0.configure('dpi.Vertical.TScrollbar', arrowsize='15p')
    st0.configure('dpi.TCombobox', arrowsize='19p')
    st0.configure('dpi.Horizontal.TProgressbar', thickness='15p')
    st0.configure('dpi.Vertical.TProgressbar', thickness='15p')
    st0.configure('dpi.Horizontal.TScale', sliderthickness='22p', arrowsize='15p',
                  sliderlength='45p' if content in ('clam', 'classic','default') else None)
    st0.configure('dpi.Vertical.TScale', sliderthickness='22p', arrowsize='15p',
                  sliderlength='34p' if content in ('clam', 'classic','default') else None)
    root.title(content)
...
st0 = Style()
#st0.configure('dpi.TSpinbox', arrowsize='2p')
st0.configure('dpi.Horizontal.TScrollbar', arrowsize='8p')
st0.configure('dpi.Vertical.TScrollbar', arrowsize='8p')
...
vsb = Scrollbar(fr0, orient="vertical", command=can.yview, style='dpi.Vertical.TScrollbar')
hsb = Scrollbar(fr0, orient="horizontal", command=can.xview, style='dpi.Horizontal.TScrollbar')
...
cb = Combobox(lF, values=themes, state="readonly", height=10, style='dpi.TCombobox')
cb.set(themes[0])
...
sc = TtkScale(lF, from_=from_, to=to, variable=scvar,
                    orient='vertical',length=200, showvalue=True,
                        ...
                        tickinterval=5, resolution=5, style='dpi.Vertical.TScale')

sc.set(value)

l1 = Spinbox(lF, from_=from_, to=to, textvariable=scvar, width=4,
             style='dpi.TSpinbox', font=(None,12))
...
sch = TtkScale(lF, from_=a, to=b, length=400, variable=schvar,
                         ...
                         tickinterval=5, resolution=5, style='dpi.Horizontal.TScale')
...
pb1var = IntVar()
pb2var = IntVar()
pbar = Progressbar(lF1, variable = pb1var, length = 150, mode ="indeterminate",
             style='dpi.Horizontal.TProgressbar', name='pb1', orient='horizontal')
pb2 = Progressbar(lF1, variable = pb2var, mode='indeterminate', name='pb2',
             style='dpi.Vertical.TProgressbar', orient='vertical')
pbar["value"] = 25
