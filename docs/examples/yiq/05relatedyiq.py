...
class RgbYiqSelect:
    ...
    def related(self, y, i, q):
        #y, i, q = yiq
        colour0 = rgb2hash(*yiq_to_rgb(0, i, q))
        colour20 = rgb2hash(*yiq_to_rgb(20, i, q))
        colour40 = rgb2hash(*yiq_to_rgb(40, i, q))
        colour60 = rgb2hash(*yiq_to_rgb(60, i, q))
        colour80 = rgb2hash(*yiq_to_rgb(80, i, q))
        colour100 = rgb2hash(*yiq_to_rgb(100, i, q))
        comp_col = rgb2hash(*yiq_to_rgb(100-y, -i, -q))
        self.rccan0['background'] = colour0
        self.rccan0.background = colour0
        self.rcl0['text'] = colour0
        self.rccan20['background'] = colour20
        self.rccan20.background = colour20
        self.rcl20['text'] = colour20
        self.rccan40['background'] = colour40
        self.rccan40.background = colour40
        self.rcl40['text'] = colour40
        self.rccan60['background'] = colour60
        self.rccan60.background = colour60
        self.rcl60['text'] = colour60
        self.rccan80['background'] = colour80
        self.rccan80.background = colour80
        self.rcl80['text'] = colour80
        self.rccan100['background'] = colour100
        self.rccan100.background = colour100
        self.rcl100['text'] = colour100
    ...
    def build(self):
        ...
        lf1.grid(column=0, row=0, sticky='n')
        ...
        rsc = TtkScale(lf1, from_=0, to=255, variable=self.rvar, orient='horizontal',
        ...
        gsc = TtkScale(lf1, from_=0, to=255, variable=self.gvar, orient='horizontal',
        ...
        bsc = TtkScale(lf1, from_=0, to=255, variable=self.bvar, orient='horizontal',
        ...
        self.rccan0 = Canvas(lf5, width=30, height=30, bd=0,
                             highlightthickness=0)
        self.rccan0.grid(column=0, row=0, sticky='n', columnspan=2)

        self.rcla0 = Label(lf5, text = "0% Y")
        self.rcla0.grid(column=0, row=1, sticky='n')

        self.rcl0 = Label(lf5, text = "")
        self.rcl0.grid(column=1, row=1, sticky='n')

        self.rccan20 = Canvas(lf5, width=30, height=30, bd=0,
                             highlightthickness=0)
        self.rccan20.grid(column=0, row=2, sticky='n', columnspan=2)

        self.rcla20 = Label(lf5, text = "20% Y")
        self.rcla20.grid(column=0, row=3, sticky='n')

        self.rcl20 = Label(lf5, text = "")
        self.rcl20.grid(column=1, row=3, sticky='n')

        self.rccan40 = Canvas(lf5, width=30, height=30, bd=0,
                             highlightthickness=0)
        self.rccan40.grid(column=0, row=4, sticky='n', columnspan=2)

        self.rcla40 = Label(lf5, text = "40% Y")
        self.rcla40.grid(column=0, row=5, sticky='n')

        self.rcl40 = Label(lf5, text = "")
        self.rcl40.grid(column=1, row=5, sticky='n')

        self.rccan60 = Canvas(lf5, width=30, height=30, bd=0,
                             highlightthickness=0)
        self.rccan60.grid(column=0, row=6, sticky='n', columnspan=2)

        self.rcla60 = Label(lf5, text = "60% Y")
        self.rcla60.grid(column=0, row=7, sticky='n')

        self.rcl60 = Label(lf5, text = "")
        self.rcl60.grid(column=1, row=7, sticky='n')

        self.rccan80 = Canvas(lf5, width=30, height=30, bd=0,
                             highlightthickness=0)
        self.rccan80.grid(column=0, row=8, sticky='n', columnspan=2)

        self.rcla80 = Label(lf5, text = "80% Y")
        self.rcla80.grid(column=0, row=9, sticky='n')

        self.rcl80 = Label(lf5, text = "")
        self.rcl80.grid(column=1, row=9, sticky='n')

        self.rccan100 = Canvas(lf5, width=30, height=30, bd=0,
                             highlightthickness=0)
        self.rccan100.grid(column=0, row=10, sticky='n', columnspan=2)

        self.rcla100 = Label(lf5, text = "100% Y")
        self.rcla100.grid(column=0, row=11, sticky='n')

        self.rcl100 = Label(lf5, text = "")
        self.rcl100.grid(column=1, row=11, sticky='n')
        ...
        al0 = Label(lf2, text='alpha')
        al0.grid(column=0, row=0, sticky='n')

        self.acan = Canvas(lf2, width=self.canvas_w, height=self.canvas_h, bd=0,
                           highlightthickness=0)
        self.acan.grid(column=1, row=0, sticky='n')

        asc = TtkScale(lf2, from_=0, to=255, variable=self.avar, orient='horizontal',
                       length=self.scale_l, command=self.ahandle, tickinterval=20)
        asc.grid(column=1, row=1, sticky='nw')

        asb = Spinbox(lf2, from_=0, to=255, textvariable=self.avar, validate='key',
        ...
        lf4.grid(column=2, row=0)

        yl0 = Label(lf4, text='luma')
        yl0.grid(column=0, row=0, sticky='s')

        self.ycan = Canvas(lf4, width=self.canvas_w, height=self.canvas_h, bd=0,
                           highlightthickness=0)
        self.ycan.grid(column=1, row=0, sticky='s')

        ysc = TtkScale(lf4, from_=0, to=100, variable=self.yvar, orient='horizontal',
                       length=self.scale_l, command=self.yhandle, tickinterval=10)
        ysc.grid(column=1, row=1, sticky='nw')

        vcmdyiq = root.register(yiq_okay)

        ysb = Spinbox(lf4, from_=0, to=100, textvariable=self.yvar, validate='key',
        ...
        il0 = Label(lf4, text='i hue')
        il0.grid(column=0, row=3)

        self.ican = Canvas(lf4, width=self.canvas_w, height=self.canvas_h, bd=0,
                           highlightthickness=0)
        self.ican.grid(column=1, row=3, sticky='s')

        isc = TtkScale(lf4, from_=-100, to=100, variable=self.ivar, orient='horizontal',
                       length=self.scale_l, command=self.ihandle, tickinterval=20)
        isc.grid(column=1, row=4, sticky='nw')

        isb = Spinbox(lf4, from_=-100, to=100, textvariable=self.ivar, validate='key',
        ...
        ql0 = Label(lf4, text='q hue')
        ql0.grid(column=0, row=6, sticky='s')

        self.qcan = Canvas(lf4, width=self.canvas_w, height=self.canvas_h, bd=0,
                           highlightthickness=0)
        self.qcan.grid(column=1, row=6, sticky='n')

        qsc = TtkScale(lf4, from_=-100, to=100, variable=self.qvar, orient='horizontal',
                       length=self.scale_l, command=self.qhandle, tickinterval=20)
        qsc.grid(column=1, row=7, sticky='nw')

        qsb = Spinbox(lf4, from_=-100, to=100, textvariable=self.qvar, validate='key',
