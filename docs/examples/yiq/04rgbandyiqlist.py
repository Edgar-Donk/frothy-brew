...
class RgbYiqSelect:
    ...
    def build(self):
        ...
        for ix, comp in enumerate(rgbcomps):
            Label(fr1, text=rgbnames[ix]).grid(row=3*ix, column=0)
            Label(fr1).grid(row=2+3*ix, column=2)
            self.rgbcans.append(Canvas(fr1, width=self.canvas_w, height=self.canvas_h,
                bd=0, highlightthickness=0))
            self.rgbcans[ix].grid(row=3*ix, column=1)
            TtkScale(fr1, from_=0, to=255, variable=rgbvars[ix], orient='horizontal',
                length=self.scale_l, command=rgbhandles[ix],
                tickinterval=20).grid(row=1+3*ix, column=1, sticky='nw')
            rgbsboxes.append(Spinbox(fr1, from_=0, to=255, textvariable=rgbvars[ix], validate='key',
        ...
        for ix, comp in enumerate(comps):
            ...
            Label(fr4).grid(row=2+3*ix, column=2)
